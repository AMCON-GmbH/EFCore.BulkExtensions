    ...
    public class TableInfo
    {
        ...
        public static string GetUniquePropertyValues<T>(T entity, List<string> propertiesNames, TypeAccessor accessor) {
            if (propertiesNames.Count == 1) {
                return accessor[entity, propertiesNames[0]].ToString();
            }

            // Seperator "null"-Char, because it ist not often used
            return string.Join("\0", propertiesNames.Select(propertyName => accessor[entity, propertyName]));
        }
